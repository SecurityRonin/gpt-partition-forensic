...
fn analyse_names_the_image() {
    ...
    assert!(
        out.contains("evidence.img"),
        "report must name the image:\n{out}"
    );
}
...
fn analyse_shows_disk_guid() {
    ...
    assert!(
        out.contains(DISK_GUID),
        "report must show the disk GUID:\n{out}"
    );
}
...
fn analyse_shows_partition_labels() {
    ...
    assert!(
        out.contains("Linux"),
        "partition label 'Linux' missing:\n{out}"
    );
    assert!(
        out.contains("EFI System"),
        "partition label 'EFI System' missing:\n{out}"
    );
}
...
fn analyse_indicates_backup_present() {
    ...
    assert!(
        out.contains("Backup"),
        "report must mention the backup GPT:\n{out}"
    );
}
...
fn analyse_flags_corrupt_header_crc() {
    ...
    assert!(
        out.contains("GPT-HDR-CRC"),
        "header-CRC anomaly code missing:\n{out}"
    );
    assert!(
        out.contains("HIGH"),
        "header-CRC severity (HIGH) missing:\n{out}"
    );
}
...
fn dump_lba1_shows_efi_part_signature() {
    ...
    assert!(
        out.contains("EFI PART"),
        "EFI PART must appear in the ASCII column:\n{out}"
    );
}
...
fn dump_raw_returns_full_512_byte_sector() {
    ...
    assert_eq!(
        &bytes[0..8],
        b"EFI PART",
        "LBA 1 must begin with the GPT signature"
    );
}
